...
class IkRig:
	def __init__(self,
		...
		if side is None :
			self.side = parser.find_element(self.rig_name)
	...
	def _build(self):
		...
		ik_ctrl_grp = bb.create_node('group', base=self.rig_name, elements=[self.element_name, 'ctrl'], side=side)
		ik_mod_grp = bb.create_node('group', base=self.rig_name, elements=[self.element_name, 'mod'], side=self.side)
		...
		cmds.poleVectorConstraint(ik_pv_ctrl, ikh, n=node_name)
	...
	def do_strech(self):
		...
		global_scale_mdv = bb.create_node(node_type='multiplyDivide', base=self.rig_name, elements=['global', 'scale'], number=self.number, side=self.side, namer=NAMER)
		cmds.connectAttr(f'{dist_perc_mdv}.ox', f'{global_scale_mdv}.i1x')
		cmds.connectAttr(f'{self.global_scale}.s{self.aim_axis}', f'{global_scale_mdv}.i2x')
		...
		cmds.addAttr( self.ik_end_ctrl, ln = attr_name, at = 'float', min = 0, max = 1, dv = 1, k = True )
		...
		cmds.connectAttr(f'{global_scale_mdv}.ox', f'{feature_switch}.c1r')
		...
		if NAME_TEMPLATE == 'hatrig':
			...
			for jnt in self.joints[1:3]:
				...
				cmds.connectAttr(f'{original_posi_mdl}.o', f'{jnt}.t{self.aim_axis}')
